...
export default function MyPageBanner(props) {
    ...
    useEffect(()=>{
        axios.all([
            ...
            axios.get(`${API.LOAD_MYPAGE_VOICELIST}`
            ,{headers : {Authorization : `Bearer ${getCookies('accessToken')}`}}),
    ...
    const dropItem = purchasedTTS.map((item, idx) => {
        return <Dropdown.Item eventKey={idx} className={classes['dropdown-item']} key = {idx}>{item.name}</Dropdown.Item>
    });
                    ...
                    <Col>
                        ...
                        <Row className={classes['dropdown-wrapper']}>
                            <Dropdown align="end" onSelect={setSelectedTTS} >
                                <Button className={classes['dropdown-content']} disabled>{purchasedTTS[selectedTTS].name}</Button>
